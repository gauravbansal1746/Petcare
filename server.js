...
app.get("/signup-process-get", function (req, resp) {
    ...
    //     requireTLS:true,
    //     auth:{
    //         user:'req.query.emailForServer',
    //         pass:'req.query.pwdForServer'
    //     }
    // });
    ...
    var mailOptions={
        from:'req.query.emailForServer',
        to:'req.query.emailForServer',
        subject:"Petcare signup form",
...
app.get("/changepwd-process-get",function(req,resp)
{
    var dataAry=[req.query.pwdForServerss,req.query.emailForServers,req.query.pwdForServers]
    dbRef.query("update users set pwd=? where emailid=? and pwd=?",dataAry,function(err)
    {
        if(err==null)
        {
           resp.send("changed password successfully");
        }
        else
        {
            resp.send(err.toString());
...
app.get("/changepwds-process-get",function(req,resp)
{
    var dataAry=[req.query.pwdForServee,req.query.emailForServe,req.query.pwdForServe]
    dbRef.query("update users set pwd=? where emailid=? and pwd=?",dataAry,function(err)
    {
        if(err==null)
        {
...
app.post("/caretaker-update-form-post",function(req,resp){
    ...
    var dataAry = [req.body.caretkrName, req.body.caretkrContact, req.body.caretkrAddress, req.body.caretkrWebsite, req.body.stt, req.body.city.trim(), req.body.caretkrPin, req.body.selPets.trim(), pic,req.body.caretkrEmail];
...
app.get("/fetch-all-caretakers-angular",function(req,resp){
    ...
    dbRef.query("select * from caretakers where city=? and selpets=?",dataAry,function(err,table) //where city=? and selpets like ?",dataAry,function(err,table)
    {
          if(err!=null)
              resp.send(err.toString());
          else
...
app.get("/fetch-all-caretakers-angulars",function(req,resp){

     
    //var dataAry=[req.query.cityforserver.trim(),req.query.petforserver.trim()];
    //console.log(req.query.cityforserver);

    dbRef.query("select * from caretakers ",function(err,table) //where city=? and selpets like ?",dataAry,function(err,table)
    {
          if(err!=null)
              resp.send(err.toString());
          else
